...
async function getSpendingRowsInRange(start: string, end: string): Promise<SpendingRow[]> {
  const rows = await db
    .select({
...
async function weekComparison(): Promise<SpendingComparison> {
  const thisWeekStart = startOfWeek(new Date());
  const lastWeekStart = addDays(thisWeekStart, -7);
  const nextWeekStart = addDays(thisWeekStart, 7);

  const rows = await getSpendingRowsInRange(toIso(lastWeekStart), toIso(nextWeekStart));
...
async function computeMonthVsMonth(
  ...
  const [curRows, prevRows] = await Promise.all([
    getSpendingRowsInRange(toIso(curRange.start), toIso(curRange.end)),
    getSpendingRowsInRange(toIso(prevRange.start), toIso(prevRange.end)),
  ]);
...
async function monthVsAverageComparison(): Promise<SpendingComparison> {
  ...
  await Promise.all(
    historicalMonths.map(async (entry) => {
      ...
      const rows = await getSpendingRowsInRange(toIso(range.start), toIso(range.end));
...
async function yearComparison(): Promise<SpendingComparison> {
  const today = new Date();
  const year = today.getFullYear();
  const prevYear = year - 1;

  const rows = await getSpendingRowsInRange(`${prevYear}-01-01`, `${year + 1}-01-01`);
...
export async function getSpendingComparison(mode: ComparisonMode): Promise<SpendingComparison> {
  const today = new Date();
  const year = today.getFullYear();
  const month = today.getMonth() + 1;
  ...
  switch (mode) {
    ...
    case "month": {
      ...
      return computeMonthVsMonth(year, month, prevYear, prevMonth, "this month", "last month");
    }
    case "month-vs-year":
      return computeMonthVsMonth(year, month, year - 1, month, "this month", "last year");
    case "month-vs-average":
      return monthVsAverageComparison();
    case "year":
